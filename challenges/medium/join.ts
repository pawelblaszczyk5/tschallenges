...
type Join<
  ArrayOfStrings extends Array<string>,
  Seperator extends string | number,
  CurrentResult extends string = ""
> = ArrayOfStrings extends [infer Head, ...infer Rest]
  ? Head extends string
    ? Rest extends Array<string>
      ? Rest extends []
        ? `${CurrentResult}${Head}`
        : Join<Rest, Seperator, `${CurrentResult}${Head}${Seperator}`>
      : CurrentResult
    : CurrentResult
  : CurrentResult;
