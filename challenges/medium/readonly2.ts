...
type MyReadonly2<
  ...
  KeysToMakeReadonly extends keyof ObjectType = keyof ObjectType
> = {
  readonly [Key in KeysToMakeReadonly]: ObjectType[Key];
} & {
  [Key in keyof ObjectType as Key extends KeysToMakeReadonly
    ? never
    : Key]: ObjectType[Key];
};
